...
def fetch_data_from_sheets(spreadsheet_id, sheet_range):
    ...
    try:
        result = api_call_with_backoff(sheets_service.spreadsheets().values().get, spreadsheetId=spreadsheet_id, range=sheet_range).execute()
        ...
        df_use['진행율'] = df_use['진행율'].astype(str).str.replace('%', '').str.strip().replace('', np.nan).astype(float)
...
def fetch_info_board_extended(spreadsheet_id):
    ...
    batch_request = sheets_service.spreadsheets().values().batchGet(
    ...
    for (rng, key), response in zip(ranges, result.get("valueRanges", [])):
        ...
        results[key] = values[0][0].strip() if values and values[0] and values[0][0] else "미정"
    logging.info(f"모델명: {results['model_name']}, 기구협력사: {results['mech_partner']}, 전장협력사: {results['elec_partner']}")
...
def process_data(df, model_name):
    ...
    if df.empty or '내용' not in df.columns:
        ...
        return pd.DataFrame(columns=['내용', '워킹데이 소요 시간', '총 워킹 소요 시간 (시간:분)', '작업 분류'])

    df_complete = df.dropna(subset=['시작 시간', '완료 시간']).copy()

    # 1) 작업시간 계산 (휴일/점심/주말 고려)
    df_complete['워킹데이 소요 시간'] = df_complete.apply(
        lambda row: calculate_working_hours_with_holidays(row['시작 시간'], row['완료 시간']),
        axis=1
    )
    ...
    # 3) 내용별 총 소요시간 요약 테이블
    task_total_time = df_complete.groupby(['내용', '작업 분류'])['워킹데이 소요 시간'].sum().reset_index()
...
def compute_occurrence_rates(df, task_total_time, avg_mapping, model_name, tolerance=2, mech_partner=None, elec_partner=None):
    """NaN 및 OT 발생률 계산."""
    categories = ["기구", "TMS_반제품", "전장", "검사", "마무리", "기타"]
    occurrence_stats = {cat: {"total_count": 0, "nan_count": 0, "ot_count": 0, "nan_tasks": [], "ot_task_details": []} for cat in categories}
    partner_stats = {"mech": {"nan_count": 0, "ot_count": 0}, "elec": {"nan_count": 0, "ot_count": 0}}
    df['진행율'] = pd.to_numeric(df['진행율'], errors='coerce')
    completed_tasks = set(df[(pd.to_numeric(df["진행율"], errors="coerce") >= 100) | (df["시작 시간"].notna() & df["완료 시간"].notna())]["내용"])
...
def calculate_progress_by_category(df, model_name):
    ...
    df['진행율'] = df.apply(
        lambda row: 100.0 if pd.isna(row['진행율']) and pd.notna(row['시작 시간']) and pd.notna(row['완료 시간'])
        else row['진행율'], axis=1
    )
...
def get_avg_time_mapping(model_name):
    ...
    try:
        avg_values = sheets_service.spreadsheets().values().get(
        ...
        if len(avg_values) <= 1:
            return {}
        return {
            row[0].strip(): parse_avg_time_string(row[1])
            if 'h' in row[1].lower() or 'm' in row[1].lower()
            else float(row[1])
            for row in avg_values[1:] if len(row) >= 2
        }
...
def get_mech_start_date(spreadsheet_url):
    ...
    try:
        ...
        result = api_call_with_backoff(sheets_service.spreadsheets().values().get, spreadsheetId=spreadsheet_id, range="정보판!B6", valueRenderOption="FORMATTED_VALUE").execute()
