...
def insert_document(conn, document, timestamp):
    ...
    try:
        cursor = conn.cursor()
        if 'info' in document and isinstance(document['info'], list) and len(document['info']) > 0:
            info_entry = document['info'][0]
            if isinstance(info_entry, dict):
                candidate_sn = info_entry.get('S/N')
                if candidate_sn and isinstance(candidate_sn, str) and candidate_sn.strip():
        ...
        if target_serial_number:
            ...
            if result:
                logger.info(f"Reusing existing document_id: {result[0]} for S/N='{target_serial_number}'.")
                return result[0]
        ...
        logger.info(f"Created new document_id: {new_document_id} (S/N for context: '{target_serial_number or 'N/A'}')")
        return new_document_id
    except KeyError as e:
        logger.error(f"KeyError during document insertion (S/N: '{target_serial_number or 'N/A'}'): Missing key {e}. Doc keys: {document.keys()}", exc_info=True)
        raise
    except psycopg2.Error as e:
        logger.error(f"Database error during document insertion (S/N: '{target_serial_number or 'N/A'}'): {e}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"Unexpected error during document insertion (S/N: '{target_serial_number or 'N/A'}'): {e}", exc_info=True)
        raise
    finally:
...
def insert_info(conn, document_id, info_data_list):
    ...
    try:
        ...
        if not info_data_list or not isinstance(info_data_list, list):
            logger.warning(f"No valid info_data (list) to insert for document_id={document_id}.")
            return

        values_to_insert = []
        for info_item in info_data_list:
            if not isinstance(info_item, dict):
                logger.warning(f"Item in info_data_list is not a dict for doc_id={document_id}. Item: {info_item}")
                continue
        ...
        logger.error(f"Unexpected error preparing info insert for document_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def insert_worksheet(conn, document_id, worksheet_data):
    ...
    try:
        ...
        if not worksheet_data:
            logger.warning(f"No data to insert into worksheet for document_id={document_id}.")
            return
        
        # 디버깅: worksheet_data 타입 확인
        logger.debug(f"insert_worksheet: worksheet_data type={type(worksheet_data)}, length={len(worksheet_data) if hasattr(worksheet_data, '__len__') else 'N/A'}")
        ...
        if isinstance(worksheet_data, str):
            ...
            try:
                ...
                logger.info(f"Successfully parsed worksheet_data from string to {type(worksheet_data)}")
        ...
        for worksheet_item in worksheet_data:
             if not isinstance(worksheet_item, dict):
                logger.warning(f"Item in worksheet_data is not a dict for doc_id={document_id}. Item: {worksheet_item}")
                continue
             values_to_insert.append((
                document_id,
        ...
        if not values_to_insert:
             logger.warning(f"No valid worksheet items to insert for document_id={document_id} after preparing.")
             return
        ...
        logger.info(f"Prepared to insert {len(values_to_insert)} worksheet records for document_id={document_id}")
    except Exception as e:
        logger.error(f"Error inserting worksheet for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def insert_task_summary(conn, document_id, task_summary_data, info_data_list):
    ...
    try:
        ...
        if not task_summary_data:
            logger.warning(f"No data to insert into task_summary for document_id={document_id}.")
            return

        title_number_from_info = "N/A"
        if info_data_list and isinstance(info_data_list, list) and len(info_data_list) > 0:
            if isinstance(info_data_list[0], dict):
                title_number_from_info = info_data_list[0].get('title number', "N/A")[:255]

        values_to_insert = []
        for task_item in task_summary_data:
            if not isinstance(task_item, dict):
                logger.warning(f"Item in task_summary_data is not a dict for doc_id={document_id}. Item: {task_item}")
                continue
            serial_number = task_item.get('S/N')
            if not serial_number:
                logger.warning(f"S/N missing in task_summary_data for doc_id={document_id}, task: {task_item.get('내용', 'Unknown Task')}. Skipping.")
                continue
            values_to_insert.append((
                document_id,
                serial_number[:128],
        ...
        if not values_to_insert:
            logger.warning(f"No valid task_summary items to insert for document_id={document_id} after filtering.")
            return
        ...
        logger.info(f"Prepared to insert {len(values_to_insert)} task_summary records for document_id={document_id}")
    except KeyError as e:
        logger.error(f"Key missing in task_summary data for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"Error inserting task_summary for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def parse_duration(duration_str: str) -> float | None:
    ...
    try:
        ...
        logger.error(f"ValueError parsing duration string: '{duration_str}' - Error: {str(ve)}", exc_info=True)
        return None
    except Exception as e:
        logger.error(f"Failed to parse duration string: '{duration_str}' - Error: {str(e)}", exc_info=True)
        return None
...
def insert_ot_details(conn, document_id, ot_details_data):
    ...
    try:
        ...
        if not ot_details_data:
            logger.warning(f"No data to insert into ot_details for document_id={document_id}.")
            return
        
        values_to_insert = []
        for ot_item in ot_details_data:
            if not isinstance(ot_item, dict) or 'duration' not in ot_item:
                logger.warning(f"Invalid ot_detail item for doc_id={document_id}: {ot_item}. Skipping.")
                continue
            
            duration_float = parse_duration(ot_item['duration'])
            if duration_float is None:
                logger.warning(f"Skipping ot_detail item due to duration parsing error for doc_id={document_id}, task='{ot_item.get('task_name')}'. Duration string: '{ot_item['duration']}'")
                continue
            values_to_insert.append((document_id, ot_item.get('task_name', '')[:255], duration_float))
            
        if not values_to_insert:
            logger.warning(f"No valid ot_details items to insert for document_id={document_id} after parsing/filtering.")
            return
        ...
        logger.info(f"Prepared to insert {len(values_to_insert)} ot_details records for document_id={document_id}")
    except Exception as e:
        logger.error(f"Error inserting ot_details for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def insert_progress_summary(conn, document_id, progress_data):
    cursor = None
    logger.debug(f"Attempting insert_progress_summary for doc_id={document_id}. Data type: {type(progress_data)}, Preview: {str(progress_data)[:200]}")
    try:
        cursor = conn.cursor()
        
        serial_number_for_document = None
        cursor.execute("SELECT serial_number FROM info WHERE document_id = %s ORDER BY info_id DESC LIMIT 1", (document_id,))
        sn_row = cursor.fetchone()
        serial_number_for_document = sn_row[0] if sn_row else None
        
        if not serial_number_for_document:
            logger.error(f"Serial number not found in 'info' table for document_id={document_id}. Cannot insert progress_summary.")
            return

        try:
            cursor.execute("DELETE FROM progress_summary WHERE document_id = %s", (document_id,))
            logger.debug(f"Cleared existing progress_summary rows for document_id={document_id}")
        except Exception as del_err:
            logger.error(f"Failed to clear old progress_summary rows for document_id={document_id}: {del_err}", exc_info=True)
            raise
        ...
        if isinstance(progress_data, dict):
            for category_key, progress_value in progress_data.items():
                try:
                    ...
                    logger.warning(f"Invalid progress value for category '{category_key}' (doc_id={document_id}, S/N={serial_number_for_document}): {progress_value}. Skipping.")
        
        elif isinstance(progress_data, list):
            for item in progress_data:
                if not isinstance(item, dict):
                    logger.warning(f"Progress_summary item is not a dict for doc_id={document_id}: {item}. Skipping.")
                    continue
                ...
                try:
                    ...
                    logger.warning(f"Invalid progress value for S/N='{sn_from_item}', category='{cat_from_item}' (doc_id={document_id}): {item.get('progress')}. Setting to 0.0.")
                    prog_val = 0.0
                
                if sn_from_item and cat_from_item:
                    final_sn = sn_from_item[:128] if sn_from_item else serial_number_for_document[:128]
                    if not final_sn:
                        logger.warning(f"S/N missing or unobtainable for progress_summary item (doc_id={document_id}): {item}. Skipping.")
                        continue
                    items_to_insert.append({'S/N': final_sn, 'category': str(cat_from_item)[:50], 'progress': prog_val})
                else:
                    logger.warning(f"S/N or category missing in progress_summary list item for doc_id={document_id}: {item}. Skipping.")
        else:
            if progress_data:
                 logger.error(f"Invalid type for progress_data for document_id={document_id}: {type(progress_data)}. Expected dict or list. Skipping.")
            else:
                 logger.warning(f"No data to insert into progress_summary for document_id={document_id} (data is None or empty).")
            return

        if not items_to_insert:
            logger.warning(f"No valid progress_summary items to insert for document_id={document_id} after processing.")
            return
        ...
        for item_data in items_to_insert:
            cursor.execute(
                ...
                """,
                (document_id, item_data['S/N'], item_data['category'], item_data['progress'])
            )
        logger.info(f"Prepared to insert/update {len(items_to_insert)} progress_summary records for document_id={document_id}")
    except psycopg2.Error as e:
        logger.error(f"Database error inserting/updating progress_summary for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"Unexpected error inserting/updating progress_summary for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def insert_stats(conn, document_id, stats_data):
    ...
    try:
        ...
        if not stats_data or not isinstance(stats_data, dict):
            logger.warning(f"No valid stats_data (dict) to insert for document_id={document_id}.")
            return
        
        values_to_insert = []
        for category, stat_item in stats_data.items():
            if not isinstance(stat_item, dict):
                logger.warning(f"Stat for category '{category}' is not a dict for document_id={document_id}. Skipping: {stat_item}")
                continue
            values_to_insert.append((
                document_id, str(category)[:50], stat_item.get('total_count'), stat_item.get('nan_count'),
                stat_item.get('completed_count'), Json(stat_item.get('nan_tasks', [])), stat_item.get('ot_count')
            ))

        if not values_to_insert:
            logger.warning(f"No valid stat items to insert for document_id={document_id} after filtering.")
            return
        ...
        logger.info(f"Prepared to insert {len(values_to_insert)} stats records for document_id={document_id}")
    except Exception as e:
        logger.error(f"Error inserting stats for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def insert_partner_stats(conn, document_id, partner_stats_data):
    ...
    try:
        ...
        if not isinstance(partner_stats_data, dict):
            logger.warning(f"partner_stats_data is not a dict for document_id={document_id}. Skipping. Data: {partner_stats_data}")
            return

        values_to_insert = []
        for partner_type, stat_item in partner_stats_data.items():
            if not isinstance(stat_item, dict):
                logger.warning(f"Stat for partner_type '{partner_type}' is not a dict for document_id={document_id}. Skipping: {stat_item}")
                continue
            values_to_insert.append((
                document_id, str(partner_type)[:50], stat_item.get('nan_count'), 
                stat_item.get('ot_count'), stat_item.get('name', '')[:100]
            ))
        
        if not values_to_insert:
            logger.warning(f"No valid partner_stat items to insert for document_id={document_id} after filtering.")
            return
        ...
        logger.info(f"Prepared to insert {len(values_to_insert)} partner_stats records for document_id={document_id}")
    except Exception as e:
        logger.error(f"Error inserting partner_stats for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def insert_additional_info(conn, document_id, additional_info_data):
    ...
    try:
        ...
        if not additional_info_data or not isinstance(additional_info_data, dict):
             logger.warning(f"No valid additional_info_data (dict) to insert for document_id={document_id}.")
             return
        cursor.execute(
            """
        ...
        logger.error(f"Error inserting additional_info for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def insert_treemap_data(conn, document_id, treemap_data):
    ...
    try:
        ...
        if not treemap_data or not isinstance(treemap_data, dict):
            logger.warning(f"No valid treemap_data (dict) to insert for document_id={document_id}.")
            return
        cursor.execute(
            """
            ...
            VALUES (%s, %s, %s)
            """,
            (document_id, Json(treemap_data.get('progress_treemap', {})), Json(treemap_data.get('task_treemap', {})))
        )
        logger.info(f"Prepared to insert treemap_data for document_id={document_id}")
    except Exception as e:
        logger.error(f"Error inserting treemap_data for doc_id={document_id}: {str(e)}", exc_info=True)
        raise
    finally:
        if cursor: cursor.close()
...
def verify_data(conn, document_id):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM documents WHERE document_id = %s", (document_id,))
        doc_count = cursor.fetchone()[0]
        cursor.execute("SELECT COUNT(*) FROM info WHERE document_id = %s", (document_id,))
        info_count = cursor.fetchone()[0]
        logger.info(f"Verification for document_id {document_id}: documents rows = {doc_count}, info rows = {info_count}")
    except Exception as e:
        logger.error(f"Error during data verification for document_id {document_id}: {e}", exc_info=True)
    finally:
        if cursor: cursor.close()
...
def load_json_to_db(json_file_path, LIMIT):
    ...
    try:
        with open(json_file_path, 'r', encoding='utf-8') as f:
            raw_data = f.read()
            logger.info(f"Read {len(raw_data)} bytes from {json_file_path}. Preview: {raw_data[:200]}")
            data = json.loads(raw_data)
            logger.info(f"Successfully parsed JSON from {json_file_path}. Top-level type: {type(data)}")
    except json.JSONDecodeError as e:
        logger.error(f"JSON parsing failed for file {json_file_path}: {e}", exc_info=True)
        raise
    except Exception as e:
    ...
    if "documents" not in data or not isinstance(data.get("documents"), list):
        logger.error(f"❌ 'documents' key missing or not a list. Keys: {list(data.keys())}, Type: {type(data.get('documents'))}")
...
if __name__ == '__main__':
    ...
    logger.info(f"Test DB: User={os.getenv('DB_USER')}, Host={os.getenv('DB_HOST')}, DB={os.getenv('DB_NAME')}")
        ...
        "documents": [
            {
                ...
                "info": [{"S/N": "TEST123", "Model Name": "ModelX", "title number": "TITLE001"}],
                "worksheet": [{"내용": "Task1", "작업 분류": "기구"}],
                "task_summary": [{"S/N": "TEST123", "내용": "Task1", "작업 분류": "기구", "워킹데이 소요 시간": 2.5}],
                "ot_details": [{"task_name": "OT1", "duration": "2시간 30분"}],
                "progress_summary": {"기구": 50.0},
                "stats": {"기구": {"total_count": 10, "nan_count": 2, "completed_count": 8, "nan_tasks": [], "ot_count": 1}},
                "partner_stats": {"mech": {"nan_count": 2, "ot_count": 1, "name": "MechPartner"}},
                "additional_info": {"model_name": "ModelX", "mech_partner": "MechPartner", "elec_partner": "ElecPartner"},
