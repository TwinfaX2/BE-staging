...
from googleapiclient.discovery import (
    build,
)  # build는 현재 코드에서 직접 사용되지 않음 (필요시 유지)
...
try:
    ...
    logger.critical(
        f"Failed to initialize Google services in google_api.py: {e}", exc_info=True
    )
...
def fetch_entire_sheet_values(spreadsheet_id, sheet_range):
    ...
    """
    logger.debug(
        f"Fetching entire sheet values for ID: {spreadsheet_id}, Range: {sheet_range}"
    )
...
def get_sheet_id_by_name(spreadsheet_id, sheet_name):
    ...
    """
    logger.debug(
        f"Getting sheetId for sheet name: '{sheet_name}' in spreadsheet: {spreadsheet_id}"
    )
...
def get_spreadsheet_title(spreadsheet_id: str) -> str | None:
    ...
    try:
        ...
        if title and isinstance(title, str) and title.strip():
            logger.info(
                f"Successfully fetched title for spreadsheet ID '{spreadsheet_id}': '{title}'"
            )
            return title.strip()
        else:
            logger.warning(
                f"Spreadsheet title is empty or not found for ID '{spreadsheet_id}'. API response: {spreadsheet_metadata}"
            )
            return None
    except HttpError as http_err:
        logger.error(
            f"Google Sheets API HTTP error while fetching title for ID '{spreadsheet_id}': {http_err}",
            exc_info=True,
        )
        return None
    except Exception as e:
        logger.error(
            f"Unexpected error while fetching title for ID '{spreadsheet_id}': {e}",
            exc_info=True,
        )
        return None
...
def get_order_no(spreadsheet_id: str) -> str | None:
    ...
    if re.match(r"^\d{6}/\d{4}/\d{4}$", title):
        logger.info(
            f"Valid title format for get_order_no (ID: {spreadsheet_id}): '{title}'"
        )
        return title
    else:
        logger.warning(
            f"Fetched spreadsheet title '{title}' (ID: {spreadsheet_id}) does not match expected 'Date/Order/SN' format."
        )
...
def upload_to_drive(file_path, folder_id=DRIVE_FOLDER_ID):
    ...
    try:
        ...
        logger.info(
            f"File '{file_name}' uploaded to Drive with ID: {file_id}. Setting permissions."
        )
        ...
        logger.error(
            f"Google Drive API HTTP error during upload of '{file_path}': {http_err}",
            exc_info=True,
        )
        return None
    except Exception as e:
        logger.error(
            f"Unexpected error during Drive upload of '{file_path}': {e}", exc_info=True
        )
        return None


def batch_update_spreadsheet(
    spreadsheet_id, requests_body
):  # 변수명 requests -> requests_body (Python 키워드와 충돌 방지)
    """
    ...
    if not sheets_service:  # 서비스 초기화 실패 대응
        ...
        return
    logger.debug(
        f"Batch updating spreadsheet ID: {spreadsheet_id} with {len(requests_body)} requests."
    )
...
def get_linked_spreadsheet_ids(spreadsheet_id, sheet_name):
    ...
    """
    logger.debug(
        f"Getting linked spreadsheet IDs from '{sheet_name}'!A:A in spreadsheet: {spreadsheet_id}"
    )
    pmmd_hyperlink_range = (
        f"'{sheet_name}'!A:A"  # 시트 이름에 공백이나 특수문자 있어도 처리
    )
    ...
    try:
        ...
        logger.error(
            f"Failed to fetch formulas from sheet '{sheet_name}' in spreadsheet '{spreadsheet_id}': {e}",
            exc_info=True,
        )
        return []
    ...
    if not formulas:
        logger.warning(
            f"No formulas found in range '{pmmd_hyperlink_range}' for spreadsheet '{spreadsheet_id}'."
        )
        return ids
    ...
    for row_idx, row_data in enumerate(formulas):
        ...
        for cell_formula in row_data:
            if isinstance(cell_formula, str) and cell_formula.upper().startswith(
                "=HYPERLINK("
            ):
                ...
                if match:
                    ...
                    logger.warning(
                        f"Could not parse spreadsheet ID from HYPERLINK formula in '{spreadsheet_id}' sheet '{sheet_name}', row {row_idx+1}: '{cell_formula}'"
                    )
    ...
    logger.info(
        f"Found {len(ids)} linked spreadsheet IDs from '{spreadsheet_id}' sheet '{sheet_name}'."
    )
    return ids
