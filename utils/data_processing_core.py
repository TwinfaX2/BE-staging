...
def parse_korean_datetime(dt_input):
    try:
        if isinstance(dt_input, (int, float)):
            ...
            formats = [
                ('%Y. %m. %d %p %I:%M:%S', r'^\d{4}\.\s*\d{1,2}\.\s*\d{1,2}\s+(?:AM|PM)\s+\d{1,2}:\d{2}:\d{2}$'),
                ('%Y. %m. %d %H:%M:%S', r'^\d{4}\.\s*\d{1,2}\.\s*\d{1,2}\s+\d{1,2}:\d{2}:\d{2}$'),
...
def process_data(df, model_name, info):
    try:
        df = df.copy()
        logging.info(f"[디버깅] process_data 입력 데이터: {df.head(5).to_dict(orient='records')}")
        ...
        if df_complete.empty:
            ...
            return pd.DataFrame(columns=['내용', '작업 분류', '워킹데이 소요 시간', '총 워킹 소요 시간 (시간:분)'])

        logging.info(f"[디버깅] 열 생성 전 df_complete columns: {df_complete.columns.tolist()}")
        ...
        logging.info(f"[디버깅] 열 생성 후 df_complete columns: {df_complete.columns.tolist()}")
        # 동일 작업의 소요 시간 누적
        df_summary = df_complete.groupby('내용').agg({
            '작업 분류': 'first',
            '워킹데이 소요 시간': 'sum'
        }).reset_index()

        # 소요 시간 포맷팅
        df_summary['총 워킹 소요 시간 (시간:분)'] = df_summary['워킹데이 소요 시간'].apply(
            lambda x: f"{int(x // 1)}시간 {int((x % 1) * 60)}분" if pd.notna(x) and x >= 0.0167 else f"{int(x * 60)}초" if pd.notna(x) else "0시간 0분"
        )
        
        result = df_summary[['내용', '작업 분류', '워킹데이 소요 시간', '총 워킹 소요 시간 (시간:분)']]
        logging.info(f"[디버깅] 워킹데이 소요 시간 계산 결과: {result.to_dict('records')}")
...
def calculate_progress_by_category(df, model_name):
    ...
    df['진행율'] = df.apply(
        lambda row: 100.0 if pd.notna(row['시작 시간']) and pd.notna(row['완료 시간'])
        else 0.0, axis=1
    )
    ...
    for category in categories:
        ...
        progress_summary[category] = round((completed_tasks / total_tasks * 100) if total_tasks > 0 else 0.0, 1)
    
    return progress_summary

def compute_occurrence_rates(df, task_summary_df, avg_mapping, model_name, tolerance=2, mech_partner=None, elec_partner=None):
    df = df.copy()
    df['작업 분류'] = df['내용'].apply(lambda x: classify_task(x, model_name) or "기타")

    # 완료된 작업 추출 (main_extract.py에서 진행율 계산 후 df에 반영됨)
    # 진행율이 100 이상이거나 시작 시간과 완료 시간이 모두 있는 경우 완료로 간주
    completed_tasks = set(df[(df["시작 시간"].notna() & df["완료 시간"].notna())]["내용"])
    ...
    for category in occurrence_stats:
        occurrence_stats[category]["completed_count"] = occurrence_stats[category]["total_count"] - occurrence_stats[category]["nan_count"]
        if category == "기구":
